...
$xdebug = new \XCloner\Composer\XdebugHandler\XdebugHandler('perf.php');
$xdebug->check();
unset($xdebug);
$dir = isset($argv[1]) ? $argv[1] : __DIR__ . '/../tests/compliance/perf';
\is_dir($dir) or die('Dir not found: ' . $dir);
// Warm up the runner
\XCloner\JmesPath\Env::search('foo', []);
$total = 0;
...
{
    $best = 99999;
    $runtime = \XCloner\JmesPath\Env::createRuntime();
    for ($i = 0; $i < 100; $i++) {
        $t = \microtime(\true);
        ...
        if (!\getenv('CACHE')) {
            ...
            if ($runtime instanceof \XCloner\JmesPath\CompilerRuntime) {
