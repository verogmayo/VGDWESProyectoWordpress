...
*/
namespace XCloner\Microsoft\Graph\Http;

if (!defined('ABSPATH') && \PHP_SAPI !== 'cli') {
    die;
}
use XCloner\GuzzleHttp\Client;
use XCloner\GuzzleHttp\Exception\BadResponseException;
use XCloner\Microsoft\Graph\Core\GraphConstants;
use XCloner\Microsoft\Graph\Core\ExceptionWrapper;
...
class GraphRequest
{
    ...
    public function setReturnType($returnClass)
    {
        ...
        if ($this->returnType == "GuzzleHttp\\Psr7\\Stream" || $this->returnType === \XCloner\Psr\Http\Message\StreamInterface::class) {
            $this->returnsStream = \true;
        } else {
    ...
    public function attachBody($obj)
    {
        ...
        if (is_string($obj) || is_a($obj, \XCloner\Psr\Http\Message\StreamInterface::class)) {
    ...
    public function execute($client = null)
    {
        ...
        try {
            $result = $client->request($this->requestType, $this->_getRequestUrl(), ['body' => $this->requestBody, 'timeout' => $this->timeout]);
        ...
        $response = new GraphResponse($this, $result->getBody(), $result->getStatusCode(), $result->getHeaders());
    ...
    public function executeAsync($client = null)
    {
        ...
        $promise = $client->requestAsync($this->requestType, $this->_getRequestUrl(), ['body' => $this->requestBody, 'timeout' => $this->timeout])->then(
            ...
            function ($result) {
                ...
                if ($this->returnType) {
                    $returnObject = $response->getResponseAsObject($this->returnType);
                }
                return $returnObject;
            },
    ...
    public function download($path, $client = null)
    {
        ...
        try {
            ...
            $client->request($this->requestType, $this->_getRequestUrl(), ['body' => $this->requestBody, 'sink' => $file, 'timeout' => $this->timeout]);
            if (is_resource($file)) {
                fclose($file);
            }
    ...
    public function upload($path, $client = null)
    {
        ...
        try {
            if (file_exists($path) && is_readable($path)) {
                $file = fopen($path, 'r');
                $stream = \XCloner\GuzzleHttp\Psr7\Utils::streamFor($file);
    ...
    {
        return ['Host' => $this->baseUrl, 'Content-Type' => 'application/json', 'SdkVersion' => 'Graph-php-' . GraphConstants::SDK_VERSION, 'Authorization' => 'Bearer ' . $this->accessToken];
    }
    /**
    ...
    {
        $clientSettings = ['base_uri' => $this->baseUrl, 'http_errors' => $this->http_errors, 'headers' => $this->headers];
