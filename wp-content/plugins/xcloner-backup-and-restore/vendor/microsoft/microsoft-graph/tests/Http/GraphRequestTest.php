...
use XCloner\Microsoft\Graph\Core\GraphConstants;
use XCloner\Microsoft\Graph\Graph;
use XCloner\Microsoft\Graph\Http\GraphRequest;
use XCloner\Microsoft\Graph\Http\Test\MockClientFactory;
class GraphRequestTest extends TestCase
{
    ...
    {
        $this->requests = array(new GraphRequest("GET", "/endpoint", "token", "baseUrl", "version"), new GraphRequest("PATCH", "/endpoint?query", "token", "baseUrl", "version"), new GraphRequest("GET", "/endpoint?query&query2", "token", "baseUrl", "version"));
        $this->defaultHeaders = array("Host" => "baseUrl", "Content-Type" => "application/json", "SdkVersion" => "Graph-php-" . GraphConstants::SDK_VERSION, "Authorization" => "Bearer token");
        $body = \json_encode(array('body' => 'content'));
        $mock = new GuzzleHttp\Handler\MockHandler([new GuzzleHttp\Psr7\Response(200, ['foo' => 'bar'], $body), new GuzzleHttp\Psr7\Response(201, ['foo' => 'bar']), new GuzzleHttp\Psr7\Response(200, ['foo' => 'bar'], $body)]);
    ...
    public function testSetReturnType()
    {
        ...
        $reflectionMethod = new \ReflectionMethod('XCloner\Microsoft\Graph\Http\GraphRequest', '_getRequestUrl');
    ...
    public function testAddHeaders()
    {
        ...
        $expectedHeaders = array("Host" => "baseUrl", "Content-Type" => "application/json", "SdkVersion" => "Graph-php-" . GraphConstants::SDK_VERSION, "Authorization" => "Bearer token", "test" => "header");
    ...
    public function testCustomHeadersOverwriteDefaults()
    {
        ...
        $request = $this->requests[0]->addHeaders($testHeader);
        $headers = $request->getHeaders();
        $expectedHeaders = array("Host" => "baseUrl", "Content-Type" => "application/x-www-form-urlencoded", "SdkVersion" => "Graph-php-" . GraphConstants::SDK_VERSION, "Authorization" => "Bearer token");
        $this->assertEquals($expectedHeaders, $headers);
    }
    ...
    {
        $reflectionMethod = new \ReflectionMethod('XCloner\Microsoft\Graph\Http\GraphRequest', 'createGuzzleClient');
        $reflectionMethod->setAccessible(\true);
    ...
    {
        $promise = $this->requests[0]->executeAsync($this->client);
        $this->assertInstanceOf(GuzzleHttp\Promise\PromiseInterface::class, $promise);
        $promise = $this->requests[1]->executeAsync($this->client);
        $this->assertInstanceOf(GuzzleHttp\Promise\PromiseInterface::class, $promise);
        $promise = $this->requests[0]->executeAsync($this->client);
        $promise2 = $this->requests[2]->executeAsync($this->client);
        $response = \XCloner\GuzzleHttp\Promise\unwrap(array($promise));
    ...
    public function testGetConcatenator()
    {
        ...
        $reflectionMethod = new \ReflectionMethod('XCloner\Microsoft\Graph\Http\GraphRequest', 'getConcatenator');
        $reflectionMethod->setAccessible(\true);
    ...
    public function testExecuteWith4xxResponse()
    {
        ...
        $client = MockClientFactory::create(['http_errors' => \true], $mockResponse);
...
\class_alias('XCloner\GraphRequestTest', 'GraphRequestTest', \false);
