...
class OpenTypeTest extends TestCase
{
    ...
    public function testOpenExtensions()
    {
        ...
        $eResult = $this->_client->createRequest("POST", "/me/extensions")->attachBody($extension)->setReturnType(Model\Extension::class)->execute();
        $this->assertEquals("Extension 1", $eResult->getProperties()["extensionName"]);
        $this->_client->createRequest("DELETE", "/me/extensions/" . $eResult->getId())->execute();
    }
    /**
     * @group functional
     */
    public function testSchemaExtensions()
    {
        ...
        $newExtension = $this->_client->createRequest("POST", "/schemaExtensions")->attachBody($extension)->setReturnType(Model\SchemaExtension::class)->execute();
        $this->assertEquals($extension->getDescription(), $newExtension->getDescription());
    }
}
\class_alias('XCloner\OpenTypeTest', 'OpenTypeTest', \false);
