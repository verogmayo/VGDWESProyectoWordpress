...
*/
namespace XCloner\Microsoft\Graph\Test;
...
class GraphTestBase
{
    ...
    {
        $body = "grant_type=" . $this->grantType . "&client_info=1" . "&client_id=" . $this->clientId . "&scope=" . $this->scopes . "&client_secret=" . $this->clientSecret;
        $ch = curl_init();
        ...
        $result = curl_exec($ch);
        $token = json_decode($result, \true)['access_token'];
        curl_close($ch);
        return $token;
    }
}
