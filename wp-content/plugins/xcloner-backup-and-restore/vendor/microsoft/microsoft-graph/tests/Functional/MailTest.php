...
class MailTest extends TestCase
{
    ...
    public function testSendMail()
    {
        ...
        $this->_client->createRequest("POST", "/me/sendmail")->attachBody($body)->execute();
        $subject = $message->getSubject();
        $mailFolderMessages = $this->_client->createRequest("GET", "/me/mailFolders/sentItems/messages?\$filter=subject eq '{$subject}'")->setReturnType(Model\Message::class)->execute();
    ...
    public function testSendMailWithAttachment()
    {
        ...
        $body = array("message" => $message);
        $this->_client->createRequest("POST", "/me/sendmail")->attachBody($body)->execute();
        $mailFolderMessages = $this->_client->createRequest("GET", "/me/mailFolders/sentItems/messages?\$filter=subject eq '\$message->getSubject()'")->setReturnType(Model\Message::class)->execute();
        $this->assertNotNull($mailFolderMessages);
    }
    /**
     * @group functional
    ...
    {
        $messageCollection = $this->_client->createRequest("GET", "/me/messages?\$filter=hasAttachments eq true")->setReturnType(Model\Message::class)->execute();
        $messageId = $messageCollection[0]->getId();
        if (\count($messageCollection) > 0) {
            $attachments = $this->_client->createRequest("GET", "/me/messages/{$messageId}/attachments")->setReturnType(Model\Attachment::class)->execute();
            $attachmentId = $attachments[0]->getId();
            $attachment = $this->_client->createRequest("GET", "/me/messages/{$messageId}/attachments/{$attachmentId}")->setReturnType(Model\FileAttachment::class)->execute();
    ...
    {
        $messageIterator = $this->_client->createCollectionRequest("GET", "/me/messages")->setReturnType(Model\Message::class);
...
\class_alias('XCloner\MailTest', 'MailTest', \false);
