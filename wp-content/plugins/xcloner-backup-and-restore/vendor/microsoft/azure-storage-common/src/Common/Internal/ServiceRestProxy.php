...
use XCloner\MicrosoftAzure\Storage\Common\Internal\RetryMiddlewareFactory;
...
use XCloner\MicrosoftAzure\Storage\Common\Internal\Http\HttpCallContext;
use XCloner\MicrosoftAzure\Storage\Common\Internal\Middlewares\MiddlewareBase;
...
class ServiceRestProxy extends RestProxy
{
    ...
    private static function createClient(array $options)
    {
        ...
        return new \XCloner\GuzzleHttp\Client(array_merge($options, array("defaults" => array("allow_redirects" => \true, "exceptions" => \true, "decode_content" => \true, "config" => ["curl" => [\CURLOPT_SSLVERSION => \CURL_SSLVERSION_TLSv1_2]]), 'cookies' => \true, 'verify' => $verify)));
    }
    /**
