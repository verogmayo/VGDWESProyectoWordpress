...
use XCloner\League\Flysystem\Cached\Storage\Memory;
...
class MemoryCacheTests extends TestCase
{
    ...
    public function testIsComplete()
    {
        ...
        $this->assertFalse($cache->isComplete('dirname', \true));
        $cache->setComplete('dirname', \true);
        $this->assertTrue($cache->isComplete('dirname', \true));
    }
    public function testCleanContents()
    {
        $cache = new Memory();
        $input = [['path' => 'path.txt', 'visibility' => 'public', 'invalid' => 'thing']];
        $expected = [['path' => 'path.txt', 'visibility' => 'public']];
    ...
    {
        $cache = new Memory();
        $input = [['path' => 'path.txt', 'visibility' => 'public', 'type' => 'file']];
        $cache->storeContents('', $input, \true);
        $contents = $cache->listContents('', \true);
        $cached = [];
    ...
    public function testFlush()
    {
        $cache = new Memory();
        $cache->setComplete('dirname', \true);
        $cache->updateObject('path.txt', ['path' => 'path.txt', 'visibility' => 'public']);
        $cache->flush();
        $this->assertFalse($cache->isComplete('dirname', \true));
    ...
    {
        $cache = new Memory();
        $json = [['path.txt' => ['path' => 'path.txt', 'type' => 'file']], ['dirname' => 'recursive']];
    ...
    {
        return [['getTimestamp', 'timestamp', 12344], ['getMimetype', 'mimetype', 'text/plain'], ['getSize', 'size', 12], ['getVisibility', 'visibility', 'private'], ['read', 'contents', '__contents__']];
    }
    /**
    ...
    public function testMetaGetters($method, $key, $value)
    {
        ...
        $cache->updateObject('path.txt', $object = ['path' => 'path.txt', 'type' => 'file', $key => $value] + Util::pathinfo('path.txt'), \true);
    ...
    {
        $cache = new Memory();
        $cache->updateObject('path.txt', ['contents' => 'something']);
    ...
    {
        $cache = new Memory();
        $cache->storeContents('dirname', [['path' => 'dirname', 'type' => 'dir'], ['path' => 'dirname/nested', 'type' => 'dir'], ['path' => 'dirname/nested/deep', 'type' => 'dir'], ['path' => 'other/nested/deep', 'type' => 'dir']], \true);
        $this->isTrue($cache->isComplete('other/nested', \true));
    }
    ...
    {
        $cache = new Memory();
        $cache->storeContents('dirname', [['path' => 'dirname/path.txt', 'type' => 'file']]);
    ...
    {
        $cache = new Memory();
        $cache->storeContents('', [['path' => 'dirname', 'type' => 'dir'], ['path' => 'dirname/file.txt', 'type' => 'file'], ['path' => 'other', 'type' => 'dir'], ['path' => 'other/file.txt', 'type' => 'file'], ['path' => 'other/nested/file.txt', 'type' => 'file']]);
        $this->assertCount(3, $cache->listContents('other', \true));
    }
    public function testComplextListContentsWithDeletedFile()
    {
        $cache = new Memory();
        $cache->storeContents('', [['path' => 'dirname', 'type' => 'dir'], ['path' => 'dirname/file.txt', 'type' => 'file'], ['path' => 'other', 'type' => 'dir'], ['path' => 'other/file.txt', 'type' => 'file'], ['path' => 'other/another_file.txt', 'type' => 'file']]);
        $cache->delete('other/another_file.txt');
        $this->assertCount(4, $cache->listContents('', \true));
    }
    public function testCacheMissIfContentsIsFalse()
    {
        $cache = new Memory();
        $cache->updateObject('path.txt', ['path' => 'path.txt', 'contents' => \false], \true);
        $this->assertFalse($cache->read('path.txt'));
    }
}
\class_alias('XCloner\MemoryCacheTests', 'MemoryCacheTests', \false);
