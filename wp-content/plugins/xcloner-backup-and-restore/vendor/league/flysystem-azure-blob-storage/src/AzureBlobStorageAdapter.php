...
namespace XCloner\League\Flysystem\AzureBlobStorage;

if (!defined('ABSPATH') && \PHP_SAPI !== 'cli') {
    die;
}
use function XCloner\GuzzleHttp\Psr7\stream_for;
...
use XCloner\MicrosoftAzure\Storage\Blob\BlobRestProxy;
use XCloner\MicrosoftAzure\Storage\Blob\Models\BlobPrefix;
use XCloner\MicrosoftAzure\Storage\Blob\Models\BlobProperties;
use XCloner\MicrosoftAzure\Storage\Blob\Models\CreateBlockBlobOptions;
use XCloner\MicrosoftAzure\Storage\Blob\Models\ListBlobsOptions;
use XCloner\MicrosoftAzure\Storage\Common\Exceptions\ServiceException;
use XCloner\MicrosoftAzure\Storage\Common\Models\ContinuationToken;
...
class AzureBlobStorageAdapter extends AbstractAdapter
{
     ...
     * @var string[]
     */
    protected static $metaOptions = ['CacheControl', 'ContentType', 'Metadata', 'ContentLanguage', 'ContentEncoding'];
    /**
     * @var BlobRestProxy
     */
    private $client;
    ...
    protected function upload($path, $contents, Config $config)
    {
        ...
        $response = $this->client->createBlockBlob($this->container, $destination, $contents, $options);
        $stream->detach();
        return ['path' => $path, 'timestamp' => (int) $response->getLastModified()->getTimestamp(), 'dirname' => Util::dirname($path), 'type' => 'file'];
    }
    ...
    public function readStream($path)
    {
        ...
        try {
            ...
            return $this->normalizeBlobProperties($path, $response->getProperties()) + ['stream' => $response->getContentStream()];
        } catch (ServiceException $exception) {
            if ($exception->getCode() !== 404) {
                throw $exception;
            }
    ...
    public function getMetadata($path)
    {
        ...
        try {
            return $this->normalizeBlobProperties($path, $this->client->getBlobProperties($this->container, $path)->getProperties());
        } catch (ServiceException $exception) {
            if ($exception->getCode() !== 404) {
                throw $exception;
            }
