...
use DomainException;
use Exception;
use XCloner\ExpiredException;
use XCloner\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use XCloner\Firebase\JWT\Key;
use XCloner\Firebase\JWT\SignatureInvalidException;
use XCloner\Google\Auth\Cache\MemoryCacheItemPool;
use XCloner\Google\Exception as GoogleException;
use XCloner\GuzzleHttp\Client;
use XCloner\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use LogicException;
use XCloner\phpseclib3\Crypt\PublicKeyLoader;
use XCloner\phpseclib3\Crypt\RSA\PublicKey;
// Firebase v2
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (class_exists('XCloner\Firebase\JWT\JWT')) {
            $jwtClass = 'XCloner\Firebase\JWT\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (class_exists('XCloner\phpseclib3\Crypt\RSA\PublicKey')) {
    ...
    {
        if (class_exists('XCloner\phpseclib3\Crypt\RSA')) {
            return 'XCloner\phpseclib3\Crypt\RSA';
        }
        if (class_exists('XCloner\phpseclib\Crypt\RSA')) {
            return 'XCloner\phpseclib\Crypt\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (class_exists('XCloner\phpseclib3\Math\BigInteger')) {
            return 'XCloner\phpseclib3\Math\BigInteger';
        }
        if (class_exists('XCloner\phpseclib\Math\BigInteger')) {
            return 'XCloner\phpseclib\Math\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (class_exists('XCloner\phpseclib3\Crypt\AES')) {
            return 'XCloner\phpseclib3\Crypt\AES::ENGINE_OPENSSL';
        }
        if (class_exists('XCloner\phpseclib\Crypt\RSA')) {
            return 'XCloner\phpseclib\Crypt\RSA::MODE_OPENSSL';
        }
        if (class_exists('XCloner\Crypt_RSA')) {
