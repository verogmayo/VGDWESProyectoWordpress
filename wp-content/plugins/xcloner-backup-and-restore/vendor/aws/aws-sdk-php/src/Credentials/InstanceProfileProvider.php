...
namespace XCloner\Aws\Credentials;

if (!defined('ABSPATH') && \PHP_SAPI !== 'cli') {
    die;
}
use XCloner\Aws\Exception\CredentialsException;
use XCloner\Aws\Exception\InvalidJsonException;
use XCloner\Aws\Sdk;
use XCloner\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = isset($config['client']) ? $config['client'] : \XCloner\Aws\default_http_handler();
    }
    /**
        ...
        return Promise\Coroutine::of(function () use ($previousCredentials) {
            ...
            while ($this->secureMode && is_null($token)) {
                try {
                    ...
                    } else if (!method_exists($e, 'getResponse') || empty($e->getResponse()) || !in_array($e->getResponse()->getStatusCode(), [400, 500, 502, 503, 504])) {
            ...
            while ($result == null) {
                try {
                    ...
                    if (($this->getExceptionStatusCode($e) === 500 || strpos($e->getMessage(), "cURL error 28") !== \false) && $previousCredentials instanceof Credentials) {
                        goto generateCredentials;
                    } else if (!empty($this->getExceptionStatusCode($e)) && $this->getExceptionStatusCode($e) === 401) {
                        $this->secureMode = \true;
                    }
                    $this->handleRetryableException($e, ['blacklist' => [401, 403]], $this->createErrorMessage($e->getMessage()));
                }
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \XCloner\Aws\default_user_agent();
