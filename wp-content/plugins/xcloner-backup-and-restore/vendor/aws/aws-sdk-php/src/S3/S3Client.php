...
use XCloner\Aws\Api\DocModel;
use XCloner\Aws\Api\Service;
use XCloner\Aws\AwsClient;
use XCloner\Aws\CacheInterface;
use XCloner\Aws\ClientResolver;
use XCloner\Aws\Command;
use XCloner\Aws\Exception\AwsException;
use XCloner\Aws\HandlerList;
use XCloner\Aws\InputValidationMiddleware;
use XCloner\Aws\Middleware;
use XCloner\Aws\Retry\QuotaManager;
use XCloner\Aws\RetryMiddleware;
use XCloner\Aws\ResultInterface;
use XCloner\Aws\CommandInterface;
use XCloner\Aws\RetryMiddlewareV2;
use XCloner\Aws\S3\UseArnRegion\Configuration;
use XCloner\Aws\S3\UseArnRegion\ConfigurationInterface;
use XCloner\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use XCloner\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use XCloner\GuzzleHttp\Exception\RequestException;
use XCloner\GuzzleHttp\Promise\Promise;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    public function createPresignedRequest(CommandInterface $command, $expires, array $options = [])
    {
        ...
        $request = \XCloner\Aws\serialize($command);
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \XCloner\Aws\serialize($command)->getUri();
    }
    /**
    ...
    {
        if ($value) {
            $config = \XCloner\Aws\Retry\ConfigurationProvider::unwrap($value);
