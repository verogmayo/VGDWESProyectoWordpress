...
use XCloner\Aws\Api\Validator;
use XCloner\Aws\Api\ApiProvider;
use XCloner\Aws\Api\Service;
use XCloner\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use XCloner\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use XCloner\Aws\ClientSideMonitoring\Configuration;
use XCloner\Aws\Credentials\Credentials;
use XCloner\Aws\Credentials\CredentialsInterface;
use XCloner\Aws\Endpoint\PartitionEndpointProvider;
use XCloner\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use XCloner\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use XCloner\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use XCloner\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use XCloner\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use XCloner\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use XCloner\Aws\EndpointDiscovery\ConfigurationInterface;
use XCloner\Aws\EndpointDiscovery\ConfigurationProvider;
use XCloner\Aws\Exception\InvalidRegionException;
use XCloner\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use XCloner\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use XCloner\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use XCloner\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use XCloner\Aws\Signature\SignatureProvider;
use XCloner\Aws\Endpoint\EndpointProvider;
use XCloner\GuzzleHttp\Promise\PromiseInterface;
use XCloner\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \XCloner\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \XCloner\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \XCloner\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            $args['region'] = \XCloner\Aws\strip_fips_pseudo_regions($args['region']);
