...
use XCloner\AWS\CRT\Auth\Signable;
use XCloner\AWS\CRT\Auth\SignatureType;
use XCloner\AWS\CRT\Auth\Signing;
use XCloner\AWS\CRT\Auth\SigningAlgorithm;
use XCloner\AWS\CRT\Auth\SigningConfigAWS;
use XCloner\AWS\CRT\Auth\StaticCredentialsProvider;
use XCloner\AWS\CRT\HTTP\Request;
use XCloner\Aws\Exception\CommonRuntimeException;
use XCloner\Aws\Exception\CouldNotCreateChecksumException;
...
{
    use SignatureTrait;
    const ISO8601_BASIC = 'XCloner\Ymd\THis\Z';
    ...
    protected function signWithV4a(CredentialsInterface $credentials, RequestInterface $request, $signingService)
    {
        ...
        $signingConfig = new SigningConfigAWS(['algorithm' => SigningAlgorithm::SIGv4_ASYMMETRIC, 'signature_type' => SignatureType::HTTP_REQUEST_HEADERS, 'credentials_provider' => $credentials_provider, 'signed_body_value' => $sha, 'region' => "*", 'service' => $signingService, 'date' => time()]);
        $sha = $this->getPayload($request);
