 ...
 */
use XCloner\AWS\CRT\HTTP\Headers;
use XCloner\AWS\CRT\HTTP\Request;
use XCloner\AWS\CRT\HTTP\Response;
...
final class HttpMessageTest extends CrtTestCase
{
    ...
    {
        $headers_array = ["host" => "s3.amazonaws.com", "test" => "this is a test header value"];
    ...
    {
        $headers = ["host" => "s3.amazonaws.com", "test" => "this is a test header value"];
        $method = "GET";
        $path = "/index.php";
        $query = [];
    ...
    public function testRequestMarshallingWithQueryParams()
    {
        ...
        $method = "GET";
        $path = "/index.php";
        $query = ['request' => '1', 'test' => 'true', 'answer' => '42', 'foo' => 'bar'];
        $msg = new Request($method, $path, $query, $headers);
        $msg_buf = Request::marshall($msg);
        $msg_copy = Request::unmarshall($msg_buf);
        $this->assertMessagesMatch($msg, $msg_copy);
    ...
    {
        $headers = ["content-length" => "42", "test" => "this is a test header value"];
        $method = "GET";
        $path = "/index.php";
        $query = ['response' => '1'];
...
\class_alias('XCloner\HttpMessageTest', 'HttpMessageTest', \false);
