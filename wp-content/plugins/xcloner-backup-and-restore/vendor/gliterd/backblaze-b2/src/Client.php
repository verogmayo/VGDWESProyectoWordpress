...
use XCloner\BackblazeB2\Exceptions\ValidationException;
...
class Client
{
    ...
    public function __construct($accountId, $applicationKey, array $options = [])
    {
        ...
        $this->authTimeoutSeconds = 12 * 60 * 60;
        // 12 hour default
    ...
    public function createBucket(array $options)
    {
        ...
        $response = $this->sendAuthorizedRequest('POST', 'b2_create_bucket', ['accountId' => $this->accountId, 'bucketName' => $options['BucketName'], 'bucketType' => $options['BucketType']]);
    ...
    public function updateBucket(array $options)
    {
        ...
        $response = $this->sendAuthorizedRequest('POST', 'b2_update_bucket', ['accountId' => $this->accountId, 'bucketId' => $options['BucketId'], 'bucketType' => $options['BucketType']]);
        return new Bucket($response['bucketId'], $response['bucketName'], $response['bucketType']);
    }
    /**
    ...
    {
        $buckets = [];
        $response = $this->sendAuthorizedRequest('POST', 'b2_list_buckets', ['accountId' => $this->accountId]);
    ...
    public function deleteBucket(array $options)
    {
        ...
        $this->sendAuthorizedRequest('POST', 'b2_delete_bucket', ['accountId' => $this->accountId, 'bucketId' => $options['BucketId']]);
        return \true;
    }
    /**
    ...
    public function upload(array $options)
    {
        ...
        $response = $this->sendAuthorizedRequest('POST', 'b2_get_upload_url', ['bucketId' => $options['BucketId']]);
        ...
        if (!isset($options['FileLastModified'])) {
            $options['FileLastModified'] = round(microtime(\true) * 1000);
        }
        ...
        return new File($response['fileId'], $response['fileName'], $response['contentSha1'], $response['contentLength'], $response['contentType'], $response['fileInfo'], $response['bucketId'], $response['action'], $response['uploadTimestamp']);
    }
    /**
    ...
    public function download(array $options)
    {
        ...
        $requestUrl = null;
        $customHeaders = $options['Headers'] ?? [];
        $requestOptions = ['headers' => array_merge(['Authorization' => $this->authToken], $customHeaders), 'sink' => isset($options['SaveAs']) ? $options['SaveAs'] : null];
        ...
        return isset($options['SaveAs']) ? \true : $response;
    }
    /**
     * Copy a file.
     *
    ...
    public function copy(array $options)
    {
        ...
        $sourceFiles = $this->listFiles(['BucketId' => $options['BucketId'], 'FileName' => $options['FileName']]);
        $sourceFileId = !empty($sourceFiles) ? $sourceFiles[0]->getId() : \false;
        if (!$sourceFileId) {
            throw new NotFoundException('Source file not found in B2');
        }
        $json = ['sourceFileId' => $sourceFileId, 'fileName' => $options['SaveAs']];
    ...
    public function listFiles(array $options)
    {
        ...
        while (\true) {
            $response = $this->sendAuthorizedRequest('POST', 'b2_list_file_names', ['bucketId' => $options['BucketId'], 'startFileName' => $nextFileName, 'maxFileCount' => $maxFileCount, 'prefix' => $prefix, 'delimiter' => $delimiter]);
    ...
    public function getFile(array $options)
    {
        if (!isset($options['FileId']) && isset($options['BucketId']) && isset($options['FileName'])) {
            $files = $this->listFiles(['BucketId' => $options['BucketId'], 'FileName' => $options['FileName']]);
            if (empty($files)) {
        ...
        $response = $this->sendAuthorizedRequest('POST', 'b2_get_file_info', ['fileId' => $options['FileId']]);
        return new File($response['fileId'], $response['fileName'], $response['contentSha1'], $response['contentLength'], $response['contentType'], $response['fileInfo'], $response['bucketId'], $response['action'], $response['uploadTimestamp']);
    }
    /**
    ...
    public function deleteFile(array $options)
    {
        ...
        $this->sendAuthorizedRequest('POST', 'b2_delete_file_version', ['fileName' => $options['FileName'], 'fileId' => $options['FileId']]);
        return \true;
    }
    /**
    ...
    public function getFileUri(array $options)
    {
        ...
        return ['Authorization' => $this->authToken, 'Uri' => $requestUri];
    }
    /**
    ...
    protected function authorizeAccount()
    {
        ...
        $response = $this->client->guzzleRequest('GET', self::B2_API_BASE_URL . self::B2_API_V1 . 'b2_authorize_account', ['auth' => [$this->accountId, $this->applicationKey]]);
    ...
    {
        $files = $this->listFiles(['BucketName' => $bucketName, 'FileName' => $fileName]);
    ...
    {
        return $this->sendAuthorizedRequest('POST', 'b2_start_large_file', ['fileName' => $fileName, 'contentType' => $contentType, 'bucketId' => $bucketId]);
    }
    /**
    ...
    {
        return $this->sendAuthorizedRequest('POST', 'b2_get_upload_part_url', ['fileId' => $fileId]);
    }
    /**
    ...
    protected function uploadParts($filePath, $uploadUrl, $largeFileAuthToken, $options = [])
    {
        ...
        while ($total_bytes_sent < $local_file_size) {
            ...
            fseek($file_handle, $total_bytes_sent);
            $customHeaders = $options['Headers'] ?? [];
            $response = $this->client->guzzleRequest('POST', $uploadUrl, ['headers' => array_merge(['Authorization' => $largeFileAuthToken, 'Content-Length' => $bytes_sent_for_part, 'X-Bz-Part-Number' => $part_no, 'X-Bz-Content-Sha1' => $sha1_of_parts[$part_no - 1]], $customHeaders), 'body' => $data_part]);
            $return[] = $response;
    ...
    {
        $response = $this->sendAuthorizedRequest('POST', 'b2_finish_large_file', ['fileId' => $fileId, 'partSha1Array' => $sha1s]);
        return new File($response['fileId'], $response['fileName'], $response['contentSha1'], $response['contentLength'], $response['contentType'], $response['fileInfo'], $response['bucketId'], $response['action'], $response['uploadTimestamp']);
    }
    /**
    ...
    {
        $this->authorizeAccount();
        return $this->client->guzzleRequest($method, $this->apiUrl . $route, ['headers' => ['Authorization' => $this->authToken], 'json' => $json]);
    }
}
