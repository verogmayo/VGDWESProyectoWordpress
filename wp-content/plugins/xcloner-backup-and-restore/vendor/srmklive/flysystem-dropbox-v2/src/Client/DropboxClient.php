...
namespace XCloner\Srmklive\Dropbox\Client;

if (!defined('ABSPATH') && \PHP_SAPI !== 'cli') {
    die;
}
use XCloner\GuzzleHttp\Client as HttpClient;
use XCloner\GuzzleHttp\Exception\ClientException as HttpClientException;
use XCloner\GuzzleHttp\Psr7\StreamWrapper;
use XCloner\Srmklive\Dropbox\Exceptions\BadRequest;
use XCloner\Srmklive\Dropbox\UploadContent;
class DropboxClient
{
    use UploadContent;
    ...
    protected function setClient(HttpClient $client = null)
    {
        if ($client instanceof HttpClient) {
            ...
            $this->client = new HttpClient(['headers' => ['Authorization' => "Bearer {$this->accessToken}"]]);
        }
    }
    /**
    ...
    {
        $this->setupRequest(['from_path' => $this->normalizePath($fromPath), 'to_path' => $this->normalizePath($toPath)]);
    ...
    {
        $this->setupRequest(['path' => $this->normalizePath($path)]);
    ...
    {
        $this->setupRequest(['path' => $this->normalizePath($path), 'format' => $format, 'size' => $size]);
    ...
    {
        $this->setupRequest(['path' => $this->normalizePath($path), 'recursive' => $recursive]);
    ...
    {
        $this->setupRequest(['cursor' => $cursor]);
    ...
    public function upload($path, $contents, $mode = 'add')
    {
        ...
        $this->setupRequest(['path' => $this->normalizePath($path), 'mode' => $mode]);
    ...
    {
        $headers = ['Dropbox-API-Arg' => json_encode($this->request)];
        if (!empty($this->content) || $this->apiEndpoint == 'files/upload_session/finish') {
    ...
    {
        try {
            $response = $this->client->post("{$this->apiContentUrl}{$this->apiEndpoint}", ['headers' => $this->setupDropboxHeaders(), 'body' => !empty($this->content) ? $this->content : '']);
        } catch (HttpClientException $exception) {
            throw $this->determineException($exception);
        }
        return $response;
