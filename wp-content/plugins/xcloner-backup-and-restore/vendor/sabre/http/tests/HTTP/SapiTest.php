...
class SapiTest extends \XCloner\PHPUnit\Framework\TestCase
{
    public function testConstructFromServerArray()
    {
        $request = Sapi::createFromServerArray(['REQUEST_URI' => '/foo', 'REQUEST_METHOD' => 'GET', 'HTTP_USER_AGENT' => 'Evert', 'CONTENT_TYPE' => 'text/xml', 'CONTENT_LENGTH' => '400', 'SERVER_PROTOCOL' => 'HTTP/1.0']);
        $this->assertEquals('GET', $request->getMethod());
        $this->assertEquals('/foo', $request->getUrl());
        $this->assertEquals(['User-Agent' => ['Evert'], 'Content-Type' => ['text/xml'], 'Content-Length' => ['400']], $request->getHeaders());
    ...
    {
        $request = Sapi::createFromServerArray(['REQUEST_URI' => '/foo', 'REQUEST_METHOD' => 'GET', 'PHP_AUTH_USER' => 'user', 'PHP_AUTH_PW' => 'pass']);
        $this->assertEquals('GET', $request->getMethod());
        $this->assertEquals('/foo', $request->getUrl());
        $this->assertEquals(['Authorization' => ['Basic ' . base64_encode('user:pass')]], $request->getHeaders());
    }
    public function testConstructPHPAuthDigest()
    {
        $request = Sapi::createFromServerArray(['REQUEST_URI' => '/foo', 'REQUEST_METHOD' => 'GET', 'PHP_AUTH_DIGEST' => 'blabla']);
        $this->assertEquals('GET', $request->getMethod());
        $this->assertEquals('/foo', $request->getUrl());
        $this->assertEquals(['Authorization' => ['Digest blabla']], $request->getHeaders());
    }
    public function testConstructRedirectAuth()
    {
        $request = Sapi::createFromServerArray(['REQUEST_URI' => '/foo', 'REQUEST_METHOD' => 'GET', 'REDIRECT_HTTP_AUTHORIZATION' => 'Basic bla']);
        $this->assertEquals('GET', $request->getMethod());
        $this->assertEquals('/foo', $request->getUrl());
        $this->assertEquals(['Authorization' => ['Basic bla']], $request->getHeaders());
    }
    /**
    ...
    {
        $request = Sapi::createFromServerArray(['SERVER_PROTOCOL' => 'HTTP/2.0', 'REQUEST_URI' => 'bla', 'REQUEST_METHOD' => 'GET']);
    ...
    public function testSendContentRangeStream($ignoreAtStart, $sendText, $multiplier, $ignoreAtEnd, $contentLength)
    {
        ...
        $response = new Response(200, ['Content-Length' => $contentLength, 'Content-Range' => sprintf('bytes %d-%d/%d', $ignoreAtStartLength, $ignoreAtStartLength + strlen($partial) - 1, $ignoreAtStartLength + strlen($partial) + $ignoreAtEndLength)]);
        $response->setBody($body);
        ob_start();
        Sapi::sendResponse($response);
        $result = ob_get_clean();
