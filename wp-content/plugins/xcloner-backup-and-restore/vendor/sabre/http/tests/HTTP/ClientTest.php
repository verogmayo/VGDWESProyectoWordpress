...
class ClientTest extends \XCloner\PHPUnit\Framework\TestCase
{
    public function testCreateCurlSettingsArrayGET()
    {
        ...
        $settings = [\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_POSTREDIR => 0, \CURLOPT_HTTPHEADER => ['X-Foo: bar'], \CURLOPT_NOBODY => \false, \CURLOPT_URL => 'http://example.org/', \CURLOPT_CUSTOMREQUEST => 'GET', \CURLOPT_USERAGENT => 'sabre-http/' . Version::VERSION . ' (http://sabre.io/)'];
    ...
    public function testCreateCurlSettingsArrayHEAD()
    {
        ...
        $settings = [\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_NOBODY => \true, \CURLOPT_CUSTOMREQUEST => 'HEAD', \CURLOPT_HTTPHEADER => ['X-Foo: bar'], \CURLOPT_URL => 'http://example.org/', \CURLOPT_USERAGENT => 'sabre-http/' . Version::VERSION . ' (http://sabre.io/)'];
        // FIXME: CURLOPT_PROTOCOLS and CURLOPT_REDIR_PROTOCOLS are currently unsupported by HHVM
        // at least if this unit test fails in the future we know it is :)
        if (\false === defined('HHVM_VERSION')) {
            $settings[\CURLOPT_PROTOCOLS] = \CURLPROTO_HTTP | \CURLPROTO_HTTPS;
    ...
    public function testCreateCurlSettingsArrayGETAfterHEAD()
    {
        ...
        $request = new Request('GET', 'http://example.org/', ['X-Foo' => 'bar']);
        $settings = [\CURLOPT_CUSTOMREQUEST => 'GET', \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_HTTPHEADER => ['X-Foo: bar'], \CURLOPT_NOBODY => \false, \CURLOPT_URL => 'http://example.org/', \CURLOPT_USERAGENT => 'sabre-http/' . Version::VERSION . ' (http://sabre.io/)'];
        // FIXME: CURLOPT_PROTOCOLS and CURLOPT_REDIR_PROTOCOLS are currently unsupported by HHVM
        // at least if this unit test fails in the future we know it is :)
        if (\false === defined('HHVM_VERSION')) {
            $settings[\CURLOPT_PROTOCOLS] = \CURLPROTO_HTTP | \CURLPROTO_HTTPS;
    ...
    public function testCreateCurlSettingsArrayPUTStream()
    {
        ...
        $settings = [\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_PUT => \true, \CURLOPT_INFILE => $h, \CURLOPT_INFILESIZE => strlen($fileContent), \CURLOPT_NOBODY => \false, \CURLOPT_CUSTOMREQUEST => 'PUT', \CURLOPT_HTTPHEADER => ['X-Foo: bar'], \CURLOPT_URL => 'http://example.org/', \CURLOPT_USERAGENT => 'sabre-http/' . Version::VERSION . ' (http://sabre.io/)'];
        // FIXME: CURLOPT_PROTOCOLS and CURLOPT_REDIR_PROTOCOLS are currently unsupported by HHVM
        // at least if this unit test fails in the future we know it is :)
        if (\false === defined('HHVM_VERSION')) {
            $settings[\CURLOPT_PROTOCOLS] = \CURLPROTO_HTTP | \CURLPROTO_HTTPS;
    ...
    public function testCreateCurlSettingsArrayPUTString()
    {
        ...
        $settings = [\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_NOBODY => \false, \CURLOPT_POSTFIELDS => 'boo', \CURLOPT_CUSTOMREQUEST => 'PUT', \CURLOPT_HTTPHEADER => ['X-Foo: bar'], \CURLOPT_URL => 'http://example.org/', \CURLOPT_USERAGENT => 'sabre-http/' . Version::VERSION . ' (http://sabre.io/)'];
        // FIXME: CURLOPT_PROTOCOLS and CURLOPT_REDIR_PROTOCOLS are currently unsupported by HHVM
        // at least if this unit test fails in the future we know it is :)
        if (\false === defined('HHVM_VERSION')) {
            $settings[\CURLOPT_PROTOCOLS] = \CURLPROTO_HTTP | \CURLPROTO_HTTPS;
    ...
    public function testIssue89MultiplePutInfileGivesWarning()
    {
        ...
        $settings = $client->createCurlSettingsArray($request);
        $this->assertEquals(\true, $settings[\CURLOPT_PUT]);
    ...
    public function testHttpErrorException()
    {
        ...
        try {
            ...
            $this->assertInstanceOf('XCloner\Sabre\HTTP\Response', $e->getResponse());
        }
    }
    public function testParseCurlResult()
    {
        ...
        $client->on('curlStuff', function (&$return) {
            $return = [['header_size' => 33, 'http_code' => 200], 0, ''];
        });
