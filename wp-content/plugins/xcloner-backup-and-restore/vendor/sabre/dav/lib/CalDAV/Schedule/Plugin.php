...
use DateTimeZone;
use XCloner\Sabre\CalDAV\ICalendar;
use XCloner\Sabre\CalDAV\ICalendarObject;
use XCloner\Sabre\CalDAV\Xml\Property\ScheduleCalendarTransp;
...
use XCloner\Sabre\DAV\Exception\NotImplemented;
...
use XCloner\Sabre\DAV\Sharing;
use XCloner\Sabre\DAV\Xml\Property\LocalHref;
...
use XCloner\Sabre\VObject\ITip;
use XCloner\Sabre\VObject\ITip\Message;
use XCloner\Sabre\VObject\Reader;
/**
...
class Plugin extends ServerPlugin
{
    ...
    public function initialize(Server $server)
    {
         ...
         */
        array_push($server->protectedProperties, $ns . 'schedule-inbox-URL', $ns . 'schedule-outbox-URL', $ns . 'calendar-user-address-set', $ns . 'calendar-user-type', $ns . 'schedule-default-calendar-URL');
    }
    /**
    ...
    public function scheduleLocalDelivery(ITip\Message $iTipMessage)
    {
        ...
        // the users' calendars.
        $uid = $iTipMessage->uid;
        $newFileName = 'sabredav-' . \XCloner\Sabre\DAV\UUIDUtil::getUUID() . '.ics';
    ...
    public function getSupportedPrivilegeSet(INode $node, array &$supportedPrivilegeSet)
    {
        $ns = '{' . self::NS_CALDAV . '}';
        if ($node instanceof IOutbox) {
            $supportedPrivilegeSet[$ns . 'schedule-send'] = ['abstract' => \false, 'aggregates' => [
                $ns . 'schedule-send-invite' => ['abstract' => \false, 'aggregates' => []],
                $ns . 'schedule-send-reply' => ['abstract' => \false, 'aggregates' => []],
                $ns . 'schedule-send-freebusy' => ['abstract' => \false, 'aggregates' => []],
                // Privilege from an earlier scheduling draft, but still
                // used by some clients.
                $ns . 'schedule-post-vevent' => ['abstract' => \false, 'aggregates' => []],
            ]];
        }
        if ($node instanceof IInbox) {
            $supportedPrivilegeSet[$ns . 'schedule-deliver'] = ['abstract' => \false, 'aggregates' => [$ns . 'schedule-deliver-invite' => ['abstract' => \false, 'aggregates' => []], $ns . 'schedule-deliver-reply' => ['abstract' => \false, 'aggregates' => []], $ns . 'schedule-query-freebusy' => ['abstract' => \false, 'aggregates' => []]]];
        }
    }
    /**
    ...
    protected function processICalendarChange($oldObject, VCalendar $newObject, array $addresses, array $ignore = [], &$modified = \false)
    {
        ...
        foreach ($messages as $message) {
            ...
            } else if (isset($newObject->VEVENT->ATTENDEE)) {
    ...
    protected function getAddressesForPrincipal($principal)
    {
        ...
        $properties = $this->server->getProperties($principal, [$CUAS]);
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Adds calendar-auto-schedule, as defined in rfc6638', 'link' => 'http://sabre.io/dav/scheduling/'];
    }
}
