...
SQL
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $calendar = ['id' => [(int) $row['calendarid'], (int) $row['id']], 'uri' => $row['uri'], 'principaluri' => $row['principaluri'], '{' . CalDAV\Plugin::NS_CALENDARSERVER . '}getctag' => 'http://sabre.io/ns/sync/' . ($row['synctoken'] ? $row['synctoken'] : '0'), '{http://sabredav.org/ns}sync-token' => $row['synctoken'] ? $row['synctoken'] : '0', '{' . CalDAV\Plugin::NS_CALDAV . '}supported-calendar-component-set' => new CalDAV\Xml\Property\SupportedCalendarComponentSet($components), '{' . CalDAV\Plugin::NS_CALDAV . '}schedule-calendar-transp' => new CalDAV\Xml\Property\ScheduleCalendarTransp($row['transparent'] ? 'transparent' : 'opaque'), 'share-resource-uri' => '/ns/share/' . $row['calendarid']];
            ...
            if ($row['access'] > 1) {
                ...
                // the future.
                $calendar['read-only'] = \XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_READ === (int) $row['access'];
            }
    ...
    {
        $fieldNames = ['principaluri', 'uri', 'transparent', 'calendarid'];
        $values = [':principaluri' => $principalUri, ':uri' => $calendarUri, ':transparent' => 0];
        ...
        $calendarId = $this->pdo->lastInsertId($this->calendarTableName . '_id_seq');
        ...
        $stmt->execute($values);
        return [$calendarId, $this->pdo->lastInsertId($this->calendarInstancesTableName . '_id_seq')];
    }
    /**
    ...
    public function deleteCalendar($calendarId)
    {
        ...
        if (\XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER === $access) {
            /**
    ...
    public function getCalendarObjects($calendarId)
    {
        ...
        foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
            $result[] = ['id' => $row['id'], 'uri' => $row['uri'], 'lastmodified' => (int) $row['lastmodified'], 'etag' => '"' . $row['etag'] . '"', 'size' => (int) $row['size'], 'component' => strtolower($row['componenttype'])];
        }
        return $result;
    }
    /**
    ...
    public function getCalendarObject($calendarId, $objectUri)
    {
        ...
        return ['id' => $row['id'], 'uri' => $row['uri'], 'lastmodified' => (int) $row['lastmodified'], 'etag' => '"' . $row['etag'] . '"', 'size' => (int) $row['size'], 'calendardata' => $row['calendardata'], 'component' => strtolower($row['componenttype'])];
    }
    /**
    ...
    public function getMultipleCalendarObjects($calendarId, array $uris)
    {
        ...
        foreach (array_chunk($uris, 900) as $chunk) {
            ...
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $result[] = ['id' => $row['id'], 'uri' => $row['uri'], 'lastmodified' => (int) $row['lastmodified'], 'etag' => '"' . $row['etag'] . '"', 'size' => (int) $row['size'], 'calendardata' => $row['calendardata'], 'component' => strtolower($row['componenttype'])];
            }
        }
        return $result;
    }
    ...
    public function createCalendarObject($calendarId, $objectUri, $calendarData)
    {
        ...
        $stmt->execute([$calendarId, $objectUri, $calendarData, time(), $extraData['etag'], $extraData['size'], $extraData['componentType'], $extraData['firstOccurence'], $extraData['lastOccurence'], $extraData['uid']]);
    ...
    protected function getDenormalizedData($calendarData)
    {
        ...
        if (!$componentType) {
            throw new \XCloner\Sabre\DAV\Exception\BadRequest('Calendar objects must have a VJOURNAL, VEVENT or VTODO component');
        }
...
SQL;
    ...
    public function getSubscriptionsForUser($principalUri)
    {
        ...
        $subscriptions = [];
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $subscription = ['id' => $row['id'], 'uri' => $row['uri'], 'principaluri' => $row['principaluri'], 'source' => $row['source'], 'lastmodified' => $row['lastmodified'], '{' . CalDAV\Plugin::NS_CALDAV . '}supported-calendar-component-set' => new CalDAV\Xml\Property\SupportedCalendarComponentSet(['VTODO', 'VEVENT'])];
    ...
    {
        $fieldNames = ['principaluri', 'uri', 'source', 'lastmodified'];
    ...
    public function getSchedulingObject($principalUri, $objectUri)
    {
        ...
        return ['uri' => $row['uri'], 'calendardata' => $row['calendardata'], 'lastmodified' => $row['lastmodified'], 'etag' => '"' . $row['etag'] . '"', 'size' => (int) $row['size']];
    }
    /**
...
INSERT INTO ' . $this->calendarInstancesTableName . '
    ...
    FROM ' . $this->calendarInstancesTableName . ' WHERE id = ?');
        foreach ($sharees as $sharee) {
            if (\XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_NOACCESS === $sharee->access) {
            ...
            if (is_null($sharee->principal)) {
                ...
                $sharee->inviteStatus = \XCloner\Sabre\DAV\Sharing\Plugin::INVITE_INVALID;
            } else {
                // Because sabre/dav does not yet have an invitation system,
                // every invite is automatically accepted for now.
                $sharee->inviteStatus = \XCloner\Sabre\DAV\Sharing\Plugin::INVITE_ACCEPTED;
            }
            ...
            $insertStmt->execute([$calendarId, $sharee->principal, $sharee->access, \XCloner\Sabre\DAV\UUIDUtil::getUUID(), $sharee->href, isset($sharee->properties['{DAV:}displayname']) ? $sharee->properties['{DAV:}displayname'] : null, $sharee->inviteStatus ?: \XCloner\Sabre\DAV\Sharing\Plugin::INVITE_NORESPONSE, $instanceId]);
        }
    }
    /**
...
SQL;
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $result[] = new Sharee([
                'href' => isset($row['share_href']) ? $row['share_href'] : \XCloner\Sabre\HTTP\encodePath($row['principaluri']),
