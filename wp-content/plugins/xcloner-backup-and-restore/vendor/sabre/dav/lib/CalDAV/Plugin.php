...
use DateTimeZone;
use XCloner\Sabre\CalDAV\Xml\Request\CalendarMultiGetReport;
...
class Plugin extends DAV\ServerPlugin
{
    ...
    public function initialize(DAV\Server $server)
    {
        ...
        $server->xml->elementMap['{' . self::NS_CALDAV . '}supported-calendar-component-set'] = 'XCloner\Sabre\CalDAV\Xml\Property\SupportedCalendarComponentSet';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}calendar-query'] = 'XCloner\Sabre\CalDAV\Xml\Request\CalendarQueryReport';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}calendar-multiget'] = 'XCloner\Sabre\CalDAV\Xml\Request\CalendarMultiGetReport';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}free-busy-query'] = 'XCloner\Sabre\CalDAV\Xml\Request\FreeBusyQueryReport';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}mkcalendar'] = 'XCloner\Sabre\CalDAV\Xml\Request\MkCalendar';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}schedule-calendar-transp'] = 'XCloner\Sabre\CalDAV\Xml\Property\ScheduleCalendarTransp';
        $server->xml->elementMap['{' . self::NS_CALDAV . '}supported-calendar-component-set'] = 'XCloner\Sabre\CalDAV\Xml\Property\SupportedCalendarComponentSet';
    ...
    public function httpMkCalendar(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($body) {
            ...
            } catch (\XCloner\Sabre\Xml\ParseException $e) {
    ...
    public function beforeWriteContent($path, DAV\IFile $node, &$data, &$modified)
    {
        ...
        $this->validateICalendar($data, $path, $modified, $this->server->httpRequest, $this->server->httpResponse, \false);
    }
    /**
    ...
    public function beforeCreateFile($path, &$data, DAV\ICollection $parentNode, &$modified)
    {
        ...
        $this->validateICalendar($data, $path, $modified, $this->server->httpRequest, $this->server->httpResponse, \true);
    }
    /**
    ...
    {
        if ($node instanceof ICalendar) {
            $supportedPrivilegeSet['{DAV:}read']['aggregates']['{' . self::NS_CALDAV . '}read-free-busy'] = ['abstract' => \false, 'aggregates' => []];
        }
    }
    /**
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Adds support for CalDAV (rfc4791)', 'link' => 'http://sabre.io/dav/caldav/'];
    }
}
