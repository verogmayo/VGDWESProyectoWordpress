...
class SharingPlugin extends DAV\ServerPlugin
{
    ...
    public function initialize(DAV\Server $server)
    {
        ...
        $this->server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}share'] = 'XCloner\Sabre\CalDAV\Xml\Request\Share';
        $this->server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}invite-reply'] = 'XCloner\Sabre\CalDAV\Xml\Request\InviteReply';
    ...
    public function propFindEarly(DAV\PropFind $propFind, DAV\INode $node)
    {
        if ($node instanceof ISharedCalendar) {
            $propFind->handle('{' . Plugin::NS_CALENDARSERVER . '}invite', function () use ($node) {
                return new Xml\Property\Invite($node->getInvites());
            });
        }
    }
    /**
    ...
    public function propFindLate(DAV\PropFind $propFind, DAV\INode $node)
    {
        if ($node instanceof ISharedCalendar) {
            ...
            if ($rt = $propFind->get('{DAV:}resourcetype')) {
                switch ($shareAccess) {
                    case \XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER:
                        $rt->add('{' . Plugin::NS_CALENDARSERVER . '}shared-owner');
                        break;
                    case \XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_READ:
                    case \XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_READWRITE:
                        $rt->add('{' . Plugin::NS_CALENDARSERVER . '}shared');
                        break;
                }
            }
            $propFind->handle('{' . Plugin::NS_CALENDARSERVER . '}allowed-sharing-modes', function () {
                return new Xml\Property\AllowedSharingModes(\true, \false);
            });
        }
    }
    /**
    ...
    public function propPatch($path, DAV\PropPatch $propPatch)
    {
        ...
        if (\XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER === $node->getShareAccess() || \XCloner\Sabre\DAV\Sharing\Plugin::ACCESS_NOTSHARED === $node->getShareAccess()) {
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Adds support for caldav-sharing.', 'link' => 'http://sabre.io/dav/caldav-sharing/'];
    }
}
