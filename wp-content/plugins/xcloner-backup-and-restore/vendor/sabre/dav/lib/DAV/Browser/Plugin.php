...
class Plugin extends DAV\ServerPlugin
{
     ...
     */
    public function __construct($enablePost = \true)
    {
    ...
    public function generateHeader($title, $path = null)
    {
        ...
        $vars = ['title' => $this->escapeHTML($title), 'favicon' => $this->escapeHTML($this->getAssetUrl('favicon.ico')), 'style' => $this->escapeHTML($this->getAssetUrl('sabredav.css')), 'iconstyle' => $this->escapeHTML($this->getAssetUrl('openiconic/open-iconic.css')), 'logo' => $this->escapeHTML($this->getAssetUrl('sabredav.png')), 'baseUrl' => $this->server->getBaseUri()];
        $html = <<<HTML
<!DOCTYPE html>
<html>
<head>
...
HTML;
    ...
    protected function serveAsset($assetName)
    {
        ...
        $map = ['ico' => 'image/vnd.microsoft.icon', 'png' => 'image/png', 'css' => 'text/css'];
    ...
    {
        $typeA = isset($a['{DAV:}resourcetype']) ? in_array('{DAV:}collection', $a['{DAV:}resourcetype']->getValue()) : \false;
        $typeB = isset($b['{DAV:}resourcetype']) ? in_array('{DAV:}collection', $b['{DAV:}resourcetype']->getValue()) : \false;
    ...
    {
        $html = new HtmlOutputHelper($this->server->getBaseUri(), $this->server->xml->namespaceMap);
    ...
    private function drawPropertyValue($html, $value)
    {
        ...
        } elseif ($value instanceof \XCloner\Sabre\Xml\XmlSerializable) {
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Generates HTML indexes and debug information for your sabre/dav server', 'link' => 'http://sabre.io/dav/browser-plugin/'];
    }
}
