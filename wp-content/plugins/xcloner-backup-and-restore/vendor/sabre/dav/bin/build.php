...
?>#!/usr/bin/env php
<?php 
$tasks = ['buildzip' => ['init', 'test', 'clean'], 'markrelease' => ['init', 'test', 'clean'], 'clean' => [], 'test' => ['composerupdate'], 'init' => [], 'composerupdate' => []];
$default = 'buildzip';
$baseDir = __DIR__ . '/../';
\chdir($baseDir);
$currentTask = $default;
...
function buildzip()
{
    ...
    $input = \json_decode(\file_get_contents(__DIR__ . '/../composer.json'), \true);
    $newComposer = ['require' => $input['require'], 'config' => ['bin-dir' => './bin'], 'prefer-stable' => \true, 'minimum-stability' => 'alpha'];
    unset($newComposer['require']['sabre/vobject'], $newComposer['require']['sabre/http'], $newComposer['require']['sabre/uri'], $newComposer['require']['sabre/event']);
    ...
    $fileNames = ['CHANGELOG.md', 'LICENSE', 'README.md', 'examples'];
