...
use Exception;
use XCloner\splitbrain\PHPArchive\ArchiveCorruptedException;
use XCloner\splitbrain\PHPArchive\ArchiveIllegalCompressionException;
use XCloner\splitbrain\PHPArchive\ArchiveIOException;
use XCloner\splitbrain\PHPArchive\FileInfoException;
class Xcloner_Scheduler
{
    ...
    public function disable_single_cron($schedule_id)
    {
        ...
        $schedule['status'] = 0;
        return $this->xcloner_container->get_xcloner_database()->update($this->scheduler_table, $schedule, array('id' => $schedule_id), array('%s', '%s'));
    }
    ...
    private function _xcloner_scheduler_callback($id, $schedule)
    {
        ...
        if (isset($schedule['backup_params']->backup_encrypt) && $schedule['backup_params']->backup_encrypt) {
            ...
            while (!$return_encrypted['finished']) {
                $return_encrypted = $this->xcloner_container->get_xcloner_encryption()->encrypt_file($backup_file, "", "", "", "", \true, \true);
        ...
        if (isset($schedule['remote_storage']) && $schedule['remote_storage'] && array_key_exists($schedule['remote_storage'], $this->xcloner_container->get_xcloner_remote_storage()->get_available_storages())) {
            ...
            if (method_exists($this->xcloner_container->get_xcloner_remote_storage(), "upload_backup_to_storage")) {
                call_user_func_array(array($this->xcloner_container->get_xcloner_remote_storage(), "upload_backup_to_storage"), array($backup_file, $schedule['remote_storage'], $schedule['backup_params']->backup_delete_after_remote_transfer));
            }
        }
    ...
    public function get_available_intervals()
    {
        ...
        $new_schedules['profile'] = ['interval' => '-1', 'display' => 'Manual Execution'];
