...
use XCloner\League\Flysystem\Filesystem;
use XCloner\League\Flysystem\Adapter\Local;
use SplFileInfo;
/**
...
class Xcloner_Filesystem
{
    ...
    public function __construct(Xcloner $xcloner_container)
    {
        ...
        try {
            ...
            $this->start_filesystem = new Filesystem($this->start_adapter, new Config(['disable_asserts' => \true]));
            $this->tmp_adapter = new Local($this->xcloner_settings->get_xcloner_tmp_path(), \LOCK_EX, '0001');
            $this->tmp_filesystem = new Filesystem($this->tmp_adapter, new Config(['disable_asserts' => \true]));
            $adapter = new Local($this->xcloner_settings->get_xcloner_tmp_path(), \LOCK_EX | \FILE_APPEND, '0001');
            $this->tmp_filesystem_append = new Filesystem($adapter, new Config(['disable_asserts' => \true]));
            $adapter = new Local($this->xcloner_settings->get_xcloner_store_path(), \LOCK_EX, '0001');
            $this->storage_filesystem = new Filesystem($adapter, new Config(['disable_asserts' => \true]));
            $this->storage_adapter = new Local($this->xcloner_settings->get_xcloner_store_path(), \FILE_APPEND, '0001');
            $this->storage_filesystem_append = new Filesystem($this->storage_adapter, new Config(['disable_asserts' => \true]));
        } catch (Exception $e) {
    ...
    public function get_latest_backups()
    {
        ...
        return array_map(static function ($item) {
            return array('basename' => $item['basename'], 'file_name' => $item['path'], 'size' => size_format($item['size']));
        }, $new_list);
    }
    /**
     * @throws FileNotFoundException
    ...
    public function get_backup_archives_list($storage_selection = "")
    {
        ...
        foreach ($list as $file_info) {
            ...
            if ($file_info['type'] == 'file' && isset($file_info['extension']) && in_array($file_info['extension'], $this->backup_archive_extensions)) {
    ...
    public function remove_tmp_filesystem()
    {
        ...
        $this->logger->debug(sprintf("Deleting the temporary storage folder %s", $tmp_path));
    ...
    public function cleanup_tmp_directories()
    {
        ...
        $tmp_filesystem = new Filesystem($adapter, new Config(['disable_asserts' => \true]));
    ...
    public function estimate_read_write_time()
    {
        ...
        try {
            ...
            $end_time = microtime(\true) - $start_time;
        ...
        foreach ($_backup_files_list as $file) {
            ...
            if ($exclude_days_string = $this->xcloner_settings->get_xcloner_option($xcloner_cleanup_exclude_days)) {
                ...
                if (in_array($backup_day_of_month, $exclude_days)) {
                    $this->logger->info(sprintf("Excluding %s from storage %s cleanup trashing as it was taken on month day %s ", $file['path'], $storage_name, $backup_day_of_month));
                    continue;
                }
            }
            //processing rule folder capacity
            $xcloner_cleanup_capacity_limit = $options_prefix . "cleanup_capacity_limit";
            if ($this->xcloner_settings->get_xcloner_option($xcloner_cleanup_capacity_limit) && $_storage_size >= $set_storage_limit = 1024 * 1024 * $this->xcloner_settings->get_xcloner_option($xcloner_cleanup_capacity_limit)) {
                //bytes
                $storage_filesystem->delete($file['path']);
                $_storage_size -= $file['size'];
                $this->logger->info("Deleting backup " . $file['path'] . " from storage system " . $storage_name . " matching rule", array("STORAGE SIZE LIMIT", $_storage_size . " >= " . $set_storage_limit));
            }
            ...
            if ($this->xcloner_settings->get_xcloner_option($xcloner_cleanup_retention_limit_days) && $current_timestamp >= $file['timestamp']) {
                $storage_filesystem->delete($file['path']);
                $this->logger->info("Deleting backup " . $file['path'] . " from storage system " . $storage_name . " matching rule", array("RETENTION LIMIT TIMESTAMP", $file['timestamp'] . " =< " . $this->xcloner_settings->get_xcloner_option($xcloner_cleanup_retention_limit_days)));
            }
            ...
            if ($this->xcloner_settings->get_xcloner_option($xcloner_cleanup_retention_limit_archives) && $_backups_counter > $this->xcloner_settings->get_xcloner_option($xcloner_cleanup_retention_limit_archives)) {
                $storage_filesystem->delete($file['path']);
                $_backups_counter--;
                $this->logger->info("Deleting backup " . $file['path'] . " from storage system " . $storage_name . " matching rule", array("BACKUP QUANTITY LIMIT " . $xcloner_cleanup_retention_limit_archives, $_backups_counter . " >= " . $this->xcloner_settings->get_xcloner_option($xcloner_cleanup_retention_limit_archives)));
            }
        }
    }
    /**
    ...
    public function estimate_reading_time($tmp_file)
    {
        ...
        return microtime(\true) - $start_time;
    }
    ...
    public function store_file($file, $storage = 'start_filesystem')
    {
        ...
        try {
            if (!$this->tmp_filesystem_append->has($this->get_included_files_handler())) {
                ...
                $start_line = "﻿" . '"Filename","Timestamp","Size","Visibility","Storage"' . \PHP_EOL;
